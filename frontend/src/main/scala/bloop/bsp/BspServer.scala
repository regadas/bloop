...
object BspServer {
  ...
  def run(
    ...
    def startServer(handle: ConnectionHandle): Task[State] = {
       ...
       *   1. An observable that pumps input from the socket `InputStream`,
       ...
       * we would not receive RST or FIN socket messages until we receive the
       * result of all the running handlers.
       */
      ...
      val cancelIfAwaitingExit: Task[Unit] = Task {
        if (!provider.exited.get) {
          /* The socket implementation of Unix Domain sockets and Windows named pipes
           * does not send RST
           *
           */
